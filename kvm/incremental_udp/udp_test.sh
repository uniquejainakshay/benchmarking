...
do
	...
	do
		...
		echo -e "\tNetperf command : $netperf_client -H $vm_ip_address -l $netperf_test_length -b $burst_size  -w 100 -i $max_iter,$min_iter -t UDP_STREAM -c -C  -- -m $j >> $netperf_output_file"

		$netperf_client -H $vm_ip_address -l $netperf_test_length -b $burst_size -w 100 -t UDP_STREAM -c -C  -- -m $j >> $netperf_output_file & 
...
bash udp_parser_inc.sh $netperf_output_file 
