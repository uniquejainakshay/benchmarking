...
echo -e "\t#mpAvgCpu\tmpMaxCpu" > $mpstat_output_file
...
do
	...
	echo -e "\tNETPERF COMMAND : $netperf_client  -H $vm_ip_address -l $netperf_test_length -t TCP_STREAM -c -C  -- -m ${arr[$i]} >> $netperf_output_file & "
	...
	do
		echo -e "\tIteration $j\n"
		$netperf_client  -H $vm_ip_address -l $netperf_test_length -t TCP_STREAM -c -C  -- -m ${arr[$i]} >> $netperf_output_file & 
		ssh -i $key_file $username@$host_ip_address "mpstat -P ALL  $netperf_test_length 1 " > mptemp

		# process the mpstat output and write to the file 
...
echo -e "#pktSz\t#thpt\t\t#lCPUtil\t#rmCPUtil" > ntemp
grep -v   "^[a-zA-Z]\+" $netperf_output_file | grep -v "^$" | awk '{print $3+42"\t"$5"\t\t"$6"\t"$7}' >> ntemp
...
pr -mts ntemp $mpstat_output_file >> gtemp
head -1 gtemp > graph_input


for i in ${arr[@]}
do
	j=`expr $i + 42 `
	cat gtemp | grep -v "^.[a-zA-Z]\+" | grep "^$j"  |  awk '{t1 += $1; t2 +=$2; t3+=$3; t4+=$4 ; t5+=$5;t6 +=$6; count++ } END { print t1/count"\t"t2/count "\t"t3/count "\t"t4/count "\t"t5/count "\t"t6/count }' >> graph_input
done

pr -mts graph_input max_th > t
rm gtemp mptemp ntemp graph_input mpstat_op netperf_op 
mv t graph_input
exit
